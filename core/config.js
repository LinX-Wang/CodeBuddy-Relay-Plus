...
const FORCE_CHINESE = process.env.CODEBUDDY_FORCE_CHINESE == null
  ? 'true'
  : (/^(1|true|yes|on)$/i.test(process.env.CODEBUDDY_FORCE_CHINESE) ? 'true' : 'false');
 ...
 * 值统一存成字符串，读取处再做类型转换。
 */
/** Region endpoints: accounts pick one at login; refresh + forward follow it. */
const REGIONS = {
  cn:   { label: 'CN',   endpoint: 'https://copilot.tencent.com', prefixPath: '/plugin' },
  intl: { label: 'Intl', endpoint: 'https://www.codebuddy.ai',    prefixPath: '/plugin' },
};

function regionConfig(region) {
  const key = (region === 'intl') ? 'intl' : 'cn';
  const r = REGIONS[key];
  return { key, label: r.label, endpoint: r.endpoint, prefixPath: r.prefixPath };
}
...
const DEFAULT_CONFIG = {
  ...
  'forceChinese': FORCE_CHINESE,      // 是否强制模型使用简体中文输出
};
...
module.exports = {
  PORT, HOST, ENDPOINT, PREFIX_PATH, PLATFORM, REGIONS, regionConfig,
