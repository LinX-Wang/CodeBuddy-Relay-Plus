...
function accountPublic(acct) {
  ...
  return {
   ...
   addedBy: acct.addedBy || acct.source || 'file',
    region: (acct.region === 'intl') ? 'intl' : 'cn',
   uid: a.uid || '',
    nickname: a.nickname || '',
...
// 为 OpenAI 兼容模型目录确定请求区域。优先显式 region，其次使用 API Key/账号绑定区域。
function requestModelRegion(req, url) {
  const q = url && url.searchParams ? url.searchParams.get('region') : '';
  if (q === 'cn' || q === 'intl') return q;
  const headerRegion = String((req.headers || {})['x-codebuddy-region'] || '').toLowerCase();
  if (headerRegion === 'cn' || headerRegion === 'intl') return headerRegion;
  try {
    const key = auth.verifyClientKey(req);
    const id = key && key.accountId;
    if (id) {
      const acct = sessionMod.findAccountByIdOrName(id);
      if (acct && (acct.region === 'cn' || acct.region === 'intl')) return acct.region;
    }
  } catch { /* 目录接口稍后仍可返回全量 */ }
  // 管理页“指定账号”模式：模型目录跟随固定账号区域。
  try {
    const pool = sessionMod.getPoolConfig();
    if (pool && pool.mode === 'pinned' && pool.pinnedId) {
      const acct = sessionMod.getAccount(pool.pinnedId);
      if (acct && (acct.region === 'cn' || acct.region === 'intl')) return acct.region;
    }
  } catch { /* ignore */ }
  return null;
...
function distMissingHtml() {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>CodeBuddy Relay Plus</title>
 ...
 if (pathname === '/login/state' && method === 'GET') {
   try {
     const name = u.searchParams.get('name') || '';
      const region = (u.searchParams.get('region') === 'intl') ? 'intl' : 'cn';
      const data = await auth.fetchAuthState(region);
      auth.pendingLogins.set(data.state, { status: 'pending', startedAt: Date.now(), name });
      auth.completeLogin(data.state, name, region);
  ...
  if (pathname === '/api/accounts/login' && method === 'POST') {
    try {
     ...
     const name = (body && typeof body.name === 'string') ? body.name.trim() : '';
      const region = (body && body.region === 'intl') ? 'intl' : 'cn';
      const data = await auth.fetchAuthState(region);
      auth.pendingLogins.set(data.state, { status: 'pending', startedAt: Date.now(), name });
      auth.completeLogin(data.state, name, region);
  ...
  if (pathname === '/v1/models') {
    ...
    util.sendJson(res, 200, models.modelsResponse(store.listModels(), store.getHiddenModels(), requestModelRegion(req, u)));
    return;
  }
  ...
  if (pathname === '/api/models' && method === 'GET') {
    // 管理页需要全量目录，由前端国内/国际页签负责展示分隔。
    util.sendJson(res, 200, { models: models.allModels(store.listModels(), store.getHiddenModels()), region: 'all' });
    return;
  }
  if (pathname === '/api/models/probe' && method === 'GET') {
    try {
      const region = requestModelRegion(req, u) || (u.searchParams.get('region') === 'intl' ? 'intl' : 'cn');
      ...
      util.sendJson(res, 200, await auth.probeModels(acct));
