...
const projection = require('./responsesProjection');
...
function responsesToChatInput(p) {
  ...
  if (p.instructions) chat.messages.push({ role: 'system', content: sanitizeForBackend(contentToText(p.instructions)) });
  ...
  if (typeof input === 'string') {
    ...
    const flushAssistant = () => {
      ...
      if (pendingAssistant.toolCalls.length) message.tool_calls = pendingAssistant.toolCalls;
      chat.messages.push(message);
      pendingAssistant = null;
    };
    const beginAssistant = (content = '') => {
    ...
    for (const item of input) {
      if (typeof item === 'string') {
        flushAssistant();
        chat.messages.push({ role: 'user', content: item });
        continue;
      }
      ...
      if (role && item.content !== undefined && (!itemType || itemType === 'message')) {
        if (role === 'assistant') {
          const content = contentToText(item.content);
          const assistant = beginAssistant(content);
          if (Array.isArray(item.tool_calls)) assistant.toolCalls.push(...item.tool_calls);
        } else {
          flushAssistant();
          const mappedRole = role === 'developer' ? 'system' : role;
          chat.messages.push({ role: mappedRole, content: contentToChat(item.content, { sanitizeText: mappedRole === 'system' }) });
        }
        continue;
      }
      ...
      if (itemType === 'message') {
        if (role === 'assistant') {
          beginAssistant(contentToText(item.content));
        } else {
          flushAssistant();
          const mappedRole = role === 'developer' ? 'system' : (role || 'user');
          chat.messages.push({ role: mappedRole, content: contentToChat(item.content, { sanitizeText: mappedRole === 'system' }) });
        }
      } else if (itemType === 'function_call') {
        beginAssistant().toolCalls.push({
        ...
        flushAssistant();
        chat.messages.push({ role: role === 'developer' ? 'system' : role, content: contentToText(item.content) });
      }
    }
    flushAssistant();
  }
  ...
  for (const key of ['stop', 'seed', 'presence_penalty', 'frequency_penalty', 'response_format', 'reasoning_effort']) {
    if (p[key] !== undefined) chat[key] = p[key];
  }
  chat.stream_options = p.stream_options || { include_usage: true };
...
function streamChatToResponses(clientRes, urlStr, headers, body, originalReq) {
  return new Promise((resolve, reject) => {
    ...
    const ensureMessage = () => {
      ...
      state.msgOutputIndex = state.toolCalls.length;
      const oi = state.msgOutputIndex;
    ...
    const onChunk = (chunk) => {
      ...
      if (typeof delta.content === 'string' && delta.content) {
        ...
        emit('response.output_text.delta', { item_id: state.msgId, output_index: state.msgOutputIndex, content_index: 0, delta: delta.content });
      }
      ...
      if (Array.isArray(delta.tool_calls)) {
        for (const tc of delta.tool_calls) {
          ...
          if (tc.id && !t.id) { t.id = tc.id; t.call_id = tc.id; }
          if (tc.function) {
            if (tc.function.name) t.name += tc.function.name;
            if (tc.function.arguments) {
              t.args += tc.function.arguments;
              emit('response.function_call_arguments.delta', { item_id: t.id, output_index: pos, delta: tc.function.arguments });
            }
          }
        }
      }
    ...
    const finish = () => {
      ...
      state.toolCalls.forEach((t, pos) => {
        emit('response.function_call_arguments.done', { item_id: t.id, output_index: pos, name: t.name, arguments: t.args });
    ...
    const req = mod.request(u, { method: 'POST', headers }, (upRes) => {
      ...
      upRes.on('data', (chunk) => {
        ...
        while ((idx = sseBoundaryIndex(buf)) !== -1) {
          ...
          for (const line of block.split('\n')) {
            ...
            if (!data) continue;
            if (data === '[DONE]') { state.sawDone = true; continue; }
      ...
      upRes.on('end', () => {
        ...
        if (!state.sawDone) {
          const error = { type: 'response.failed', sequence_number: state.seq++, response: { id: state.responseId, object: 'response', status: 'failed', error: { code: 'incomplete_stream', message: '上游流式响应未收到结束标记' } } };
          clientRes.write(`event: response.failed\ndata: ${JSON.stringify(error)}\n\n`);
          clientRes.end();
          resolve({ usage: state.usage, model: state.model, status: 'error', finishReason: state.finishReason, toolCalls: state.toolCalls.length, contentLength: state.content.length, content: state.content });
          return;
        }
        finish();
        resolve({
          usage: state.usage,
          model: state.model,
          status: upRes.statusCode === 200 ? 'ok' : 'error',
...
async function handleResponses(req, res) {
  ...
  let chatPayload = responsesToChatInput(payload);
  const cfg = store.getConfig();
  if (cfg.forceModel) chatPayload.model = cfg.forceModel;
  const projected = projection.projectBody(chatPayload);
  chatPayload = projected[0];
  const projectionStats = projected[1];
  language.applyChineseInstruction(chatPayload, cfg.forceChinese !== 'false');
  chatPayload.stream = true; // CodeBuddy 只支持流式

  const timeoutMs = store.getRequestTimeoutMs();
  const inputTail = Array.isArray(payload.input)
    ? payload.input.slice(-3).map((item) => typeof item === 'string' ? 'string' : (item && item.type) || (item && item.role) || typeof item)
    : typeof payload.input;
  logger.log('info', 'responses', `model=${payload.model || chatPayload.model} stream=${!!payload.stream} messages=${chatPayload.messages.length}`, logger.requestSummary(payload, {
    messages: chatPayload.messages.length,
    inputTail,
  ...
  let acct;
  try { acct = await auth.pickAccountForRequest(accountKey, keyCheck.accountId || '', models.getModelRegion(chatPayload.model, store.listModels())); }
  catch (e) {
  ...
  try {
    if (payload.stream) {
      ...
      logger.log('info', 'responses', `流式结束 (${Date.now() - startedAt}ms)`, logger.requestSummary(payload, {
        stream: true,
        durationMs: Date.now() - startedAt,
        finishReason: done && done.finishReason,
        toolCalls: done && done.toolCalls,
        outputLength: done && done.contentLength,
        maxOutputTokens: payload.max_output_tokens || null,
        outputPreview: done && done.content ? done.content.slice(0, 180) : '',
      }));
