  ...
  try {
   ...
   if (!cols.includes('auto_checkin')) {
    ...
    if (!cols.includes('region')) {
      db.exec("ALTER TABLE accounts ADD COLUMN region TEXT NOT NULL DEFAULT 'cn'");
    }
  } catch { /* 表不存在或已就绪则忽略 */ }
  return db;
}
...
function publicValues(cfg) {
  ...
  return {
    ...
    forceChinese: asBool(c.forceChinese, true),
...
function applyPublicPatch(body) {
  ...
  if (typeof body.forceChinese === 'boolean') patch.forceChinese = String(body.forceChinese);
  // 是否校验 API 密钥
...
function insertAccount(acct) {
 ...
 const now = Date.now();
  const region = (acct.region === 'intl') ? 'intl' : 'cn';
 getDb().prepare(
    'INSERT INTO accounts(id, name, source, added_by, account, auth, accounts, auto_checkin, region, last_used_at, use_count, created_at, updated_at) ' +
    'VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ' +
   ...
   'account=excluded.account, auth=excluded.auth, accounts=excluded.accounts, auto_checkin=excluded.auto_checkin, ' +
    'region=excluded.region, last_used_at=excluded.last_used_at, use_count=excluded.use_count, updated_at=excluded.updated_at'
 ).run(
...
function updateAccountRow(id, patch) {
 ...
 getDb().prepare(
    'UPDATE accounts SET name=?, source=?, added_by=?, account=?, auth=?, accounts=?, auto_checkin=?, region=?, last_used_at=?, use_count=?, updated_at=? WHERE id=?'
 ).run(
   String(next.name || ''),
