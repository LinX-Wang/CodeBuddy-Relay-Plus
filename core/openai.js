...
async function handleProxy(req, res, pathname) {
  ...
  language.applyChineseInstruction(payload, cfg.forceChinese !== 'false');
  ...
  let acct;
  try { acct = await auth.pickAccountForRequest(accountKey, keyCheck.accountId || '', models.getModelRegion(payload.model, store.listModels())); }
  catch (e) {
  ...
  const targetUrl = `${rc.endpoint}${upstreamPath}`;
    ...
    if (needAggregate) {
      ...
      const inspectChunk = (chunk) => {
        ...
        while ((idx = streamBuffer.search(/\r?\n\r?\n/)) !== -1) {
          ...
          streamBuffer = streamBuffer.slice(idx + (streamBuffer[idx] === '\r' ? 4 : 2));
          for (const line of block.split('\n')) {
            const data = line.trim().startsWith('data:') ? line.trim().slice(5).trim() : '';
            if (!data || data === '[DONE]') continue;
            try { const obj = JSON.parse(data); if (obj && obj.usage) usage = obj.usage; } catch { /* 分片 JSON 留给后续 */ }
          }
        }
      };
      await util.pipeToClient(res, targetUrl, {
        method: 'POST', headers, body: jsonBody,
        extraHeaders: { 'Content-Type': 'text/event-stream; charset=utf-8', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'no-cache, no-transform', 'X-Accel-Buffering': 'no' },
        onChunk: inspectChunk,
