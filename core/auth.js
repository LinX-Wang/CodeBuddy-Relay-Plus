...
function buildNoAuthHeaders(region) {
  const rc = config.regionConfig(region);
 return {
   'X-No-Authorization': 'true',
   'X-No-User-Id': 'true',
   'X-No-Enterprise-Id': 'true',
   'X-No-Department-Info': 'true',
    'X-Domain': rc.endpoint.replace('https://', ''),
   'User-Agent': 'CodeBuddy-Proxy/1.0',
 };
}

function authPath(sub, region) {
  const rc = config.regionConfig(region);
  return rc.endpoint + '/v2' + rc.prefixPath + sub;
}
...
async function refreshToken(acct) {
 ...
 const headers = {
   ...
   'X-Auth-Refresh-Source': 'plugin',
    'X-Domain': acct.auth.domain || config.regionConfig(acct.region).endpoint.replace('https://', ''),
   'Content-Type': 'application/json',
   'User-Agent': 'CodeBuddy-Proxy/1.0',
 };
  const r = await util.requestJson(authPath('/auth/token/refresh', acct.region), { method: 'POST', headers, body: {}, timeoutMs: 30000 });
 ...
 * @param {string} [preferredRegion] 模型要求的账号区域
 */
async function pickAccountForRequest(explicitKey, keyAccountId, preferredRegion) {
  ...
  if (explicitKey) {
    ...
    if (candidates.length) {
      ...
      acct = candidates[cursor];
    } else {
      acct = sessionMod.pickAccount(null);
    }
  }
  if (!acct) throw new Error('未登录，请先打开管理页登录');
  if (preferredRegion && acct.region !== preferredRegion) {
    throw new Error(`模型 ${preferredRegion === 'intl' ? '国际版' : '国内版'} 与当前账号区域不匹配，请切换对应区域账号`);
  }
...
/** 使用指定账号向其区域后端探测真实模型目录。不同版本插件路径不同，依次兼容。 */
async function probeModels(acct) {
  const valid = await getValidAccount(acct);
  ...
  for (const p of paths) {
    try {
      const r = await util.requestJson(rc.endpoint + p, { method: 'GET', headers, timeoutMs: 15000 });
      if (r.status >= 200 && r.status < 300 && r.json) {
        const raw = Array.isArray(r.json) ? r.json
          : (Array.isArray(r.json.data) ? r.json.data
            : (Array.isArray(r.json.models) ? r.json.models
              : (r.json.data && Array.isArray(r.json.data.models) ? r.json.data.models : null)));
        if (Array.isArray(raw)) return { region: valid.region, endpoint: rc.endpoint + p, models: raw };
      }
      errors.push(`${p}:${r.status}`);
    } catch (e) { errors.push(`${p}:${e.message}`); }
  }
  throw new Error('未找到上游模型目录接口（' + errors.join(', ') + ')');
}

/* ============================ OAuth 登录流程 ============================ */

async function fetchAuthState(region) {
  const r = await util.requestJson(authPath('/auth/state', region) + '?platform=' + encodeURIComponent(config.PLATFORM), {
    method: 'POST', headers: buildNoAuthHeaders(region), body: {}, timeoutMs: 15000,
  });
  const data = r.json && r.json.data;
...
async function pollAuthToken(state, region) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const r = await util.requestJson(authPath('/auth/token', region) + '?state=' + encodeURIComponent(state), {
        method: 'GET', headers: buildNoAuthHeaders(region), timeoutMs: 15000,
      });
      const data = r.json && r.json.data;
      if (r.json && r.json.code === 0 && data && data.accessToken) {
        if (!data.expiresAt && data.expiresIn) data.expiresAt = Date.now() + data.expiresIn * 1000;
...
async function fetchAccount(state, auth, region) {
  const headers = Object.assign({}, buildNoAuthHeaders(region), { 'Authorization': 'Bearer ' + auth.accessToken });
  ...
  const r = await util.requestJson(authPath('/login/account', region) + '?state=' + encodeURIComponent(state), {
...
async function fetchAccounts(auth, region) {
  ...
  const r = await util.requestJson(authPath('/accounts', region), { method: 'GET', headers, timeoutMs: 15000 });
...
async function importByRefreshToken(refreshToken, name, domain) {
 ...
 const headers = {
  ...
  const region = (dom === config.regionConfig('intl').endpoint.replace('https://', '')) ? 'intl' : 'cn';
  const r = await util.requestJson(authPath('/auth/token/refresh', region), { method: 'POST', headers, body: {}, timeoutMs: 30000 });
  const data = r.json && r.json.data;
  if (!r.json || r.json.code !== 0 || !data || !data.accessToken) {
...
async function fetchAccountByToken(auth) {
  const accountRegion = auth.domain === config.regionConfig('intl').endpoint.replace('https://', '') ? 'intl' : 'cn';
  const headers = Object.assign({}, buildNoAuthHeaders(accountRegion), { 'Authorization': 'Bearer ' + auth.accessToken });
  delete headers['X-No-Authorization'];
  delete headers['X-No-User-Id'];
  delete headers['X-No-Enterprise-Id'];
  delete headers['X-No-Department-Info'];
  const r = await util.requestJson(authPath('/login/account', auth.domain === config.regionConfig('intl').endpoint.replace('https://', '') ? 'intl' : 'cn'), { method: 'GET', headers, timeoutMs: 15000 });
  if (!r.json || r.json.code !== 0 || !r.json.data) {
    throw new Error('获取账号失败: ' + (r.json ? (r.json.msg || r.json.code) : r.body));
  }
  return r.json.data;
