...
function checkinUrl(sub, region) {
  return config.regionConfig(region).endpoint + '/v2/billing/meter/' + sub;
}
...
async function checkinStatus(accountId) {
  ...
  const r = await util.requestJson(checkinUrl('checkin-activity-status', account.region), {
...
async function dailyCheckin(accountId) {
  const { headers, account } = await pickHeaders(accountId);
  const r = await util.requestJson(checkinUrl('daily-checkin', account.region), {
    method: 'POST', headers, body: {}, timeoutMs: 20000,
  });
  const json = r.json || {};
  const msg = json.msg || json.message || ('HTTP ' + r.status);
