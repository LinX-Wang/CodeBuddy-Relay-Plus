...
function openAdd() {
  ...
  Object.assign(form, {
    ...
    tools: true, vision: false, reasoning: false, region: activeRegion.value,
  });
  showForm.value = true;
}
  ...
  <div>
    <div class="card">
      ...
      <div class="model-tabs" role="tablist">
        <button class="model-tab" :class="{ active: activeRegion === 'cn' }" @click="activeRegion = 'cn'">{{ t('models.cnTab') }} <span>{{ allModels.filter(m => (m.region || 'cn') === 'cn').length }}</span></button>
        <button class="model-tab" :class="{ active: activeRegion === 'intl' }" @click="activeRegion = 'intl'">{{ t('models.intlTab') }} <span>{{ allModels.filter(m => m.region === 'intl').length }}</span></button>
      </div>
...
<style scoped>
.model-tabs { display:flex; gap:8px; margin: 14px 0 16px; border-bottom:1px solid var(--border); }
.model-tab { border:0; background:transparent; color:var(--text-2); padding:9px 16px; cursor:pointer; border-bottom:2px solid transparent; font-size:13px; }
.model-tab.active { color:var(--accent); border-bottom-color:var(--accent); font-weight:600; }
.model-tab span { margin-left:5px; opacity:.7; font-size:11px; }
