...
async function doAdd() {
  ...
  try {
    const d = await api.accountLogin(name, addRegion.value);
  ...
  <div>
    <div class="card">
      ...
      <div v-else class="table-wrap">
        <table class="table">
          <thead>
            <tr>
              <th>{{ t('accounts.colName') }} / {{ t('accounts.region') }}</th>
          ...
          <tbody>
            <tr v-for="a in accounts" :key="a.id" :class="{ pinned: pool.mode === 'pinned' && pool.pinnedId === a.id }">
              <td class="strong">
                ...
                <span class="badge" :class="a.region === 'intl' ? 'badge-intl' : 'badge-cn'">{{ a.region === 'intl' ? 'Intl' : 'CN' }}</span>
                {{ a.name || '-' }}
              </td>
              <td>{{ a.nickname || '-' }}</td>
              <td><code class='uid-cell'>{{ a.uid || '-' }}</code></td>
    ...
    <div v-if="showAdd" class="card add-card">
      ...
      <div class="region-tabs">
        <button class="region-tab" :class="{ active: addRegion === 'cn' }" @click="addRegion = 'cn'">{{ t('accounts.regionCn') }}</button>
        <button class="region-tab" :class="{ active: addRegion === 'intl' }" @click="addRegion = 'intl'">{{ t('accounts.regionIntl') }}</button>
      </div>
...
.uid-cell { max-width: 110px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: inline-block; vertical-align: middle; }
...
.region-tabs { display: flex; margin: 10px 0; border: 1px solid var(--border-strong); border-radius: 8px; overflow: hidden; width: fit-content; }
.region-tab { padding: 7px 18px; font-size: 13px; background: transparent; border: none; cursor: pointer; color: var(--text-2); }
.region-tab.active { background: var(--primary-soft); color: var(--primary); font-weight: 600; }
.badge-cn { background: rgba(59, 130, 246, 0.12); color: #3b82f6; }
.badge-intl { background: rgba(168, 85, 247, 0.12); color: #a855f7; }
